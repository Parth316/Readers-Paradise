...
const PackedOrders: React.FC = () => {
  ...
  const [isListView, setIsListView] = useState<boolean>(() => {
    return localStorage.getItem("packedOrderView") === "list"; // Separate key for packed orders
  });
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        setError(err.response?.data?.message || "Failed to fetch packed orders");
        setLoading(false);
        toast.error("Could not load packed orders.");
      }
    };
  ...
  useEffect(() => {
    localStorage.setItem("packedOrderView", isListView ? "list" : "grid");
  }, [isListView]);
  ...
  if (loading) {
    return (
      <div className="bg-gray-100 flex items-center justify-center py-4">
        <p className="text-gray-700 text-lg animate-pulse">Loading packed orders...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800">Packed Orders</h1>
      ...
      {filteredOrders.length === 0 ? (
        <div className="text-center bg-white p-8 rounded-lg shadow-md">
          <p className="text-gray-700 text-lg mb-4">
            {searchQuery ? "No packed orders match your search." : "No orders have been packed yet."}
          </p>
        </div>
      ) : isListView ? (
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {filteredOrders.map((order) => (
                <tr key={order._id} className="border-t bg-gray-200">
        ...
        <div className="space-y-6">
          {filteredOrders.map((order) => (
            <div
              key={order._id}
              className="rounded-lg shadow-md p-6 border border-gray-200 bg-gray-200"
            >
